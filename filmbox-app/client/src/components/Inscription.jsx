...
function LoginRegister() {
    ...
    const [emailErr, setEmailErr] = useState(false);    //error if the email isnt valid
    const [pwdError, setPwdError] = useState(false);    //error if the password isnt valid
    const [phoneNumberError, setPoneNumberError] = useState(false);    //error if the password isnt valid
    const [nameError, setNameError] = useState(false);    //error if the password isnt valid
    ...
    return (
        ...
        >
            ...
            <div className="wrapper">
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '5rem' }}>
                        ...
                        <div className="input-box" id="blocForm haut" style={{ gridColumn: '1/2', marginTop: '2rem' }}>
                            ...
                            <input id="inputForm" className='w-100 p-2 pe-5 ps-3 text-white border-1 rounded-4 ' type="text" placeholder="Ex: Lila" required value={firstName} onChange={(e) => setFirstName(e.target.value)}
                                ...
                                {nameError && <p className="text-danger mt-1">Only letters allowed</p>}
                        </div>
                ...
                {phoneNumberError && <p>Incorrect phone number, please put only numbers</p>}
                {nameError && <p>Invalid name or last name, only use letters</p>}
                { }
            </div>
